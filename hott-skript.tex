...
\begin{definition}
  ...
  \item Sei $f:A\to B$ eine Funktion. Wir sagen, dass $f$ eine \begriff{Quasi-Inverse} oder \begriff{Inverse} hat,
    wenn der Typ
    \begin{mathpar}
      \mathrm{qinv(f)}:\equiv\sum_{g:B\to A}\left(\left(\prod_{x:A}g(f(x))=x\right) \times \left(\prod_{y:B}f(g(y))=y\right)\right)
      \end{mathpar}
      einen Term hat.
  \end{enumerate}
\end{definition}
...
\begin{definition}
  ...
  \item Für $b:B$ ist die \begriff{Faser} von $f$ über $b$ gegeben durch:
    \begin{mathpar}
      \mathrm{fib}^{-1}(b):\equiv f^{-1}(b):\equiv \sum_{x:A}f(x)=b
    \end{mathpar}
...
Wir beginnen mit einer scheinbar unbedeutenden Variation des Begriffs Quasi-Inverse:

\begin{definition}
  Seien $A,B:\mU$ und $f:A\to B$.
  \begin{enumerate}
  \item $f$ hat eine \begriff{Rechtsinverse} oder einen \begriff{Schnitt}, wenn es ein $g:B\to A$ gibt und $f\circ g\sim \id_B$.
  \item $f$ hat eine \begriff{Linksinverse}, wenn es ein $h:B\to A$ gibt und $h\circ f\sim\id_A$.
  \item $f$ ist eine \begriff{Äquivalenz} oder eine Funktion mit \begriff{Links- und Rechtsinversen}, wenn
    \begin{mathpar}
      \mathrm{LRInv}(f):\equiv \left(\sum_{g:B\to A}\sum_{y:B}f(g(y))=y\right)\times \left(\sum_{h:B\to A}\sum_{x:A}h(f(x))=x\right)
    \end{mathpar}
  \end{enumerate}
\end{definition}
...
Die beiden Begriffe sind aber \begriff{logisch äquivalent}, d.h. es gibt für jedes $f:A\to B$ Abbildungen $\mathrm{LRInv}(f)\to \qinv(f)$ und $\qinv(f)\to\mathrm{LRInv}(f)$.

\begin{bemerkung}
  \label{bem:qinv-equiv}
  Seien $A,B$ Typen und $f:A\to B$.
  Wenn $f$ eine beidseitige Inverse hat, dann ist $f$ eine Äquivalenz. Das heißt es gibt eine Funktion:
    \begin{mathpar}
      \qinv(f)\to \isEquiv(f)
    \end{mathpar}
\end{bemerkung}
...
\begin{beweis}[von \labelcref{bem:qinv-equiv}]
  \begin{enumerate}
  \item Wir haben also $g:B\to A$, sodass $H:f\circ g\sim\id$ und $K:g\circ f\sim \id$ und müssen für jede Faser von $f$ eine Kontraktion angeben.
    Als Kontraktionszentrum für die Faser über $y:B$ wählen wir
    \begin{mathpar}
      k_y:\equiv (g(y), H(g(y))) : f^{-1}(y)
    \end{mathpar}
    Seien nun also $(x,q),(x',q'):f^{-1}(y)$. Also haben wir auch $q\kon q'^{-1}:f(x)=f(x')$ und damit:
    \begin{mathpar}
      K(x)^{-1}\kon g(q\kon q'^{-1}) \kon K(x'):x=x'
    \end{mathpar}
  \end{enumerate}
\end{beweis}
