...
\end{mathpar}
Bevor wir weitere Regeln diskutieren, machen wir zunächst ein Beispiel:
\begin{beispiel}
  Für $x,y:\einheit$ können wir nun den Typ $x=_{\einheit} y$ formen.
  ...
  \end{mathpar}
\end{beispiel}
Die Eliminationsregel, bzw die \begriff{Induktion für Gleichheit}, die auch \begriff{Pfadinduktion} genannt wird,
stellt sich als erstaunlich vielseitig heraus.
Sie besagt, dass für einen abhängigen Typen über dem Gleichheitstyp,
also etwa $p:x=_A y\yields B(x)$ und eine Vorgabe für $\refl_x$ also ein Element $b_r : \prod_{x:A} B(\refl_x)$ bereits eine abhängige Funktion wie folgt gegeben ist:
\begin{mathpar}
  \ind{=}(B,b_r) : \prod_{x:A}\prod_{y:A}\prod_{p:x=_A y}B(p)
\end{mathpar}
