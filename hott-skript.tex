...
weil es sich statt einer Aussage die gilt, eben um eine Operation handelt, die Dinge produziert.
...
\begin{definition}
  Seien $A$ ein Typ und $x:A\yields B(x)$ ein abhängiger Typ über $A$. Dann sei die Abbildung
  \begin{mathpar}
    \mathrm{tr}(p):B(x)\to B(y)
  \end{mathpar}
  der \begriff{Transport} in $B$ gegeben durch $\mathrm{tr}(\refl_x):\equiv \id_{B(x)}$.
\end{definition}
