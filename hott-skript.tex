...
\end{mathpar}
Damit können wir Werte der Funktion aus \cref{bsp:verdopplung} berechnen:
\begin{beispiel}
  Für die Funktion $d:\N\to\N$ aus \cref{bsp:verdopplung} ergibt sich mit den $\beta$-Regeln nun etwa:
  \begin{align*}
    d(3)&\equiv d(\sucN(2))    & \\
...
\begin{definition}
  \begin{enumerate}
  \item Die Abbildung $+:\N\to\N\to\N$ ist gegeben durch die folgenden Urteilsgleichungen:
    \begin{align*}
      0 + k &:\equiv k \\
      \sucN(n) + k &:\equiv \sucN(n + k)
    \end{align*}
    \begin{mathpar}
      +:\equiv\rec{\N}(\N\to\N, \id_{\N}, f \mapsto (k \mapsto \sucN(f(k))))
    \end{mathpar}
  \item Die Multiplikation ist gegeben durch:
    \begin{align*}
      0 \cdot k :\equiv 0  \\
