...
Sie haben über sogenannte ``Pull requests'' die Möglichkeit, Fehler auch selbst zu korriegieren. 
...
Wenn $B(x)$ eigentlich gar nicht von $x:A$ abhängt, also $x$ nicht in $B$ vorkommt, schreiben wir auch einfach nur $B$ und $A\to B$ für das abhängige Produkt $\prod_{x:A}B$.
