...
\begin{definition}
  ...
  \begin{enumerate}
  \item Eine \begriff{punktierte Abbildung} oder \begriff{Abbildung punktierter Typen}\index{$\to^\cdot$} ist eine Funktion $f:A\to B$ zusammen mit einer Gleichheit $p_f:f(\ast_A)=\ast_B$.
...
\item Für ein punktierte Abbildung $(f,p_f):(A,\ast_A)\to^\cdot (B,\ast_B)$ ist
  \begin{mathpar}
    \Omega(f) \colonequiv (p:\ast_A =\ast_A) \mapsto p_f^{-1}\kon f(p) \kon p_f
  \end{mathpar}
  \end{enumerate}
\end{definition}

\begin{definition}[punktierte Homotopien]
Seien $f,g:A\to^\cdot B$ punktierte Abbildungen. Eine \begriff{punktierte Homotopie} zwischen $f$ und $g$  
\end{definition}
