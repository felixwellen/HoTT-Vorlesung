...
\begin{lemma}
  ...
  \item Für jedes $Y$ ist die Abbildung, die eine Funktion $h:Y\to X$ abbildet auf den $Y$-Kegel
    \begin{center}
    \begin{tikzcd}
      Y\arrow[d,"\varphi\circ h"']\arrow[r,"\psi\circ h"] & B\arrow[d,"g"] \\
      A\arrow[r,"f",swap] & C 
    \end{tikzcd}
    \end{center}
    mit Homotopie $H_h$, eine Äquivalenz.
  \item Jeder $Y$-Kegel $(\varphi',\psi',H')$ faktorisiert eindeutig über $X$. Das heißt, dass der Typ der Abbildungen $h:Y\to X$ zusammen mit Homotopien $\varphi\circ h\sim\varphi'$, $\psi\circ h\sim\psi'$, $H_h\sim H'$ kontrahierbar ist.
