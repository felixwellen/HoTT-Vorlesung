...
\begin{lemma}
  Sei $n\geq -1$. Ein punktierter Typ $A$ ist ein $n$-Typ, wenn für alle $x:A$ der Typ $\Omega^{n+1}(A,x)$ kontrahierbar ist.
\end{lemma}
\begin{beweis}
  Sei $n\equiv -1$. Dann ist zu zeigen: $A$ ist eine Aussage, wenn $A$ kontrahierbar ist.
  Für den Induktionsschritt müssen wir zeigen: $A$ ist ein $(n+1)$-Typ, wenn $\Omega^{n+2}(A,x)$ für alle $x:A$ kontrahierbar ist.
  Nach \cref{lem:schleifenraum-n-typ} reicht es zu zeigen, dass $\Omega(A,x)$ ein $n$ Typ ist. Nach der Induktionshypothese ist dieser Typ ein $n$-Typ, wenn
...
\end{beweis}
\begin{korollar}
  Sei $n\geq -1$. Ein Typ $A$ ist ein $n$-Typ, wenn für alle $x:A$, der Typ $(S^{n+1},\ast)\to^\ast(A,x)$ kontrahierbar ist.
\end{korollar}
\begin{beweis}
...
\begin{beweis}[\cref{thm:n-truncation}]
  Per Konstruktion ist der Raum der punktierten Abbildungen $S^{n+1}\to^\ast\|A\|_n$ kontrahierbar: Als Kontraktionszentrum wählen wir die konstante Abbildung $c_\ast\colonequiv x\mapsto \ast $, die durch $\refl_\ast$ punktiert ist.
