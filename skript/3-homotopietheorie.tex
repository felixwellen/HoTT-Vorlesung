...
\begin{theorem}[Pullback-Pasting]
  Seien
  \begin{center}
    \begin{tikzcd}
      Y\ar[r]\ar[d] & X\arrow[r,"\varphi"]\arrow[d,"\psi",swap] & A\arrow[d,"f"] \\
      D\ar[r] & B\arrow[r,"g",swap] & C 
    \end{tikzcd}
  \end{center}
  ...
  \item  Durch Komposition der Homotopien gibt es auch ein drittes Quadrat der Gestalt:
    \begin{center}
      \begin{tikzcd}
        Y\ar[rr]\ar[d] &  & A\arrow[d,"f"] \\
        D\ar[rr] &  & C 
      \end{tikzcd}
    \end{center}
  \item Das dritte Quadrat ist genau dann ein Pullback, wenn die beiden kleinen Quadrate Pullbacks sind.
  \end{enumerate}
\end{theorem}
\begin{beweis}
...
\begin{theorem}[Faser-Sequenz]
  Sei $f:A\to B$ eine punktierte Abbildung. Dann gibt es eine Sequenz:
  \begin{center}
    \begin{tikzcd}
      \Omega A \ar[r,"-\Omega f"] & \Omega B \ar[r] & f^{-1}(\ast) \ar[r,"\pi"] & A \ar[r,"f"] & B
    \end{tikzcd}
  \end{center}
...
\begin{beweis}
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  Durch Pullback-Pasting wissen wir, dass $\mathrm{PB}(\ast,\pi)$ auch Pullback des zusammengesetzen Quadrats ist.
  Aber auch $\Omega B$, oder etwas genauer $f(\ast)=\ast$ ist Pullback des zusammengesetzen Quadrats, also können wir $\mathrm{PB}(\ast,\pi)$ äquivalent ersetzen durch $\Omega B$.
  Im so entstandenen Diagram, nehmen wir gleich wieder einen Pullback:
  \begin{center}
    \begin{tikzcd}
      \mathrm{PB}(\dots,\ast)\ar[r]\ar[d] & \Omega B\ar[r]\ar[d] & \eins\ar[d,"\ast"] \\
      \eins\ar[r] &f^{-1}(\ast) \ar[r,"\pi"]\ar[d] & A \ar[d,"f"] \\
      & \eins\ar[r,"\ast",swap] & B
    \end{tikzcd}
  \end{center}
\end{beweis}
