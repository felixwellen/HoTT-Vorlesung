...
\end{bemerkung}
\begin{beweis}
  Das wird durch den dritten Konstruktor direkt so erzwungen. Wir wollen hier darauf verzichten, das Rekursionsprinzip herzuleiten.
\end{beweis}
...
\begin{definition}
  ...
  \item Für jeden Typ $A$ sei der Typ $\isNType{n}$ wie folgt per Rekursion über $n:\N_{-2}$ definiert:
    \begin{align*}
      ...
      \isNType{n}(A)&\colonequiv \prod_{x,y:A}\isNType{n}(x=_A y)
    \end{align*}
...
\begin{beweis}
  ...
  Dann gibt es eine abhängige Funktion $z:\prod_{x:A}$, die jedes $x:A$ auf das Kontraktionszentrum von $B(x)$ abbildet.
  ...
  Wir zeigen nun die eigentliche Aussage per Induktion über den Homotopielevel.
...
\end{lemma}
\begin{beweis}
  Induktion über den Homotopielevel.
  ...
  \begin{mathpar}
    \isNType{(n+1)}(A)\equiv \prod_{x,y:A}\isNType{x=_A y}
  \end{mathpar}
  Für festes $x:A$ ist also mit \cref{lem:pi-n-type} der Typ $\prod_{y:A}\isNType{x=_A y}$ eine Aussage.
  Mit erneutem Anwenden von \cref{lem:pi-n-type}, ist also auch $\prod_{x:A}\prod_{y:A}\isNType{x=_A y}$ eine Aussage.
\end{beweis}
...
\begin{korollar}
  Die logischen Äquivalenzen in \cref{thm:aequivalenzen} sind Äquivalenzen.
\end{korollar}
\begin{beweis}
  Die 
\end{beweis}
