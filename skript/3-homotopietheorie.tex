...
\begin{beweis}
  ...
  \begin{mathpar}
    \mathrm{PB}(\ast,\pi_1)\equiv \left(\sum_{z:\eins}\sum_{y:\sum_{x:A}f(x)=\ast}\ast=\pi_1(y)\right)\simeq \sum_{x:A} (f(x)=\ast)\times (\ast=x)
  \end{mathpar}
  Letzterer Typ ist nun äquivalent zu $f(\ast)=\ast$ durch: $(x,p,q)\mapsto f(q)\kon p$, da $f(q)\kon p$ der Wert der zusammengesetzten Homotopie im großen Quadrat ist.
  Die Projektion von $f(\ast)=\ast$ nach $f^{-1}(\ast)$ muss also $(r:f(\ast)=\ast)\mapsto (\ast,r)$ sein. Die Äquivalenz zu $\ast=\ast$ bzw. $\Omega B$ ist durch die Punktierung $p_f:f(\ast)=\ast$ von $f$ gegeben.
  Die gesuchte Abbildung ist also $(p:\Omega B)\mapsto (\ast,p_f\kon p)$.
