...
\begin{exercise}[Schnitte von $\tilde{S^1}$]
  ...
  \begin{enumerate}
    ...
    \item $\neg\left(\prod_{x:S^1}\ast =_{S^1} x\right)$
      {\tiny Tipp: Sonst ließe sich folgern, dass jeder Typ eine Menge ist, sogar $\mU$. Warum ist das unmöglich?}
  \end{enumerate}
\end{exercise}
...
\begin{exercise}[Funktorialität der Verschleifung]
  ...
  \begin{enumerate}
    ...
    \item Zeige, dass $\Omega(g,p_g)\circ\Omega(f,p_f) \sim \Omega(g\circ f,p_{g\circ f})$ als Abbildungen $\Omega(A,a)\to \Omega(C,c)$.
    \item \textbf{Bonus.} Mit Funktionsextensionalität sind die beiden Abbildungen sogar gleich.
...
\begin{bonus}[Universelle Eigenschaft von Pullbacks II]
  ...
  f(h)^{-1} \kon H_\vartheta \kon g(k) = H'.
  $$
